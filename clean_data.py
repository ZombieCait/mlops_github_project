...
def clean_data(input_path: str, output_path: str):
    ...
    # Column actually contains -1 and -2 values presumably for studio
    # apartments.
    ...
    df.loc[
        (df["kitchen_area"] >= MAX_KITCHEN)
        | (df["kitchen_area"] <= MIN_KITCHEN),
        "kitchen_area",
    ] = 0
    ...
    df.loc[(df["kitchen_area"] == 0) & (df["rooms"] != 0), "kitchen_area"] = (
        df.loc[(df["kitchen_area"] == 0) & (df["rooms"] != 0), "area"]
        * kitchen_share
    )
...
if __name__ == "__main__":
    clean_data("data/all_v2.csv", "data/dataset.csv")
